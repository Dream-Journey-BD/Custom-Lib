...
public class GPUImageToneCurveFilter extends GPUImageFilter {
    ...
    private short readShort(@NonNull InputStream input) throws IOException {
...
//            java.nio.Buffer pixels);
    ...
    private ArrayList<Float> createSplineCurve(PointF[] points) {
        ...
        ArrayList<Float> preparedSplinePoints = new ArrayList<>(
                splinePoints != null ? splinePoints.size() : 0
        );
        if (splinePoints != null) {
            for (Point newPoint : splinePoints) {
                ...
                if (origPoint.y > newPoint.y) distance = -distance;
...
//    int n = [points count];
        int n = sdA != null ? sdA.size() : 0;
        if (n < 1) return null;
        double[] sd = new double[n];

        // From NSMutableArray to sd[n];
        for (int i = 0; i < n; i++) sd[i] = sdA.get(i);
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            for (int x = cur.x; x < next.x; x++) {
                ...
                double y = a * cur.y + t * next.y + (h * h / 6) *
                        ((a * a * a - a) * sd[i] +
                                (t * t * t - t) * sd[i + 1]);
    ...
    @Nullable
    private ArrayList<Double> createSecondDerivative(@NonNull Point[] points) {
        int n = points.length;
        if (n <= 1) {
            return null;
        }
