...
public final class BitmapUtils {
    ...
    @NonNull
    static RotateBitmapResult rotateBitmapByExif(Bitmap bitmap, @NonNull ExifInterface exif) {
        int degrees;
        int orientation =
    ...
    static BitmapSampled cropBitmapObjectHandleOOM(
            Bitmap bitmap, float[] points, int degreesRotated,
            boolean fixAspectRatio, int aspectRatioX,
            int aspectRatioY, boolean flipHorizontally,
    ...
    private static Bitmap cropBitmapObjectWithScale(
            @NonNull Bitmap bitmap, float[] points, int degreesRotated,
            boolean fixAspectRatio, int aspectRatioX, int aspectRatioY,
            float scale, boolean flipHorizontally, boolean flipVertically
    ) {

        Rect rect = getRectFromPoints(
                points,
                bitmap.getWidth(),
                bitmap.getHeight(),
                fixAspectRatio,
        ...
        if (result == bitmap && bitmap.getConfig() != null) result = bitmap.copy(
                bitmap.getConfig(), false
        );

        if (degreesRotated % 90 != 0) {
            result = cropForRotatedImage(
                    result, points, rect, degreesRotated,
    ...
    static BitmapSampled cropBitmap(
            Context context, Uri loadedImageUri, float[] points,
            int degreesRotated, int orgWidth, int orgHeight, boolean fixAspectRatio,
            int aspectRatioX, int aspectRatioY, int reqWidth,
            int reqHeight, boolean flipHorizontally, boolean flipVertically
    ) {
        int sampleMulti = 1;
        while (true) {
            try {
                return cropBitmap(
                        context, loadedImageUri, points, degreesRotated, orgWidth,
                        orgHeight, fixAspectRatio, aspectRatioX, aspectRatioY,
                        reqWidth, reqHeight, flipHorizontally, flipVertically,
                        sampleMulti
                );
            } catch (OutOfMemoryError e) {
                sampleMulti *= 2;
                if (sampleMulti > 16) {
                    throw new RuntimeException("Failed to handle OOM by sampling ("
                            + sampleMulti
                            + "): "
                            + loadedImageUri
                            + "\r\n"
    ...
    public static float getRectLeft(@NonNull float[] points) {
        return Math.min(Math.min(Math.min(points[0], points[2]), points[4]), points[6]);
    }


    public static float getRectTop(@NonNull float[] points) {
        return Math.min(Math.min(Math.min(points[1], points[3]), points[5]), points[7]);
    }


    public static float getRectRight(@NonNull float[] points) {
        return Math.max(Math.max(Math.max(points[0], points[2]), points[4]), points[6]);
    }


    public static float getRectBottom(@NonNull float[] points) {
    ...
    public static Rect getRectFromPoints(
            float[] points, int imageWidth, int imageHeight,
            boolean fixAspectRatio, int aspectRatioX, int aspectRatioY
    ) {
        ...
        if (fixAspectRatio) fixRectForAspectRatio(rect, aspectRatioX, aspectRatioY);

        return rect;
    }


    private static void fixRectForAspectRatio(
            Rect rect, int aspectRatioX, int aspectRatioY
    ) {
        if (aspectRatioX == aspectRatioY && rect.width() != rect.height()) {
            if (rect.height() > rect.width()) rect.bottom -= rect.height() - rect.width();
            else rect.right -= rect.width() - rect.height();
        }
    }
    ...
    public static Uri writeTempStateStoreBitmap(Context context, Bitmap bitmap, Uri uri) {
        try {
            ...
            if (uri == null) {
                uri = Uri.fromFile(File.createTempFile(
                        "aic_state_store_temp", ".jpg",
                        context.getCacheDir())
                );
            } else if (
                    uri.getPath() != null &&
    ...
    public static void writeBitmapToUri(
            @NonNull Context context, @NonNull Bitmap bitmap, Uri uri,
    ...
    private static BitmapSampled cropBitmap(
            Context context, Uri loadedImageUri, float[] points, int degreesRotated,
            int orgWidth, int orgHeight, boolean fixAspectRatio, int aspectRatioX,
            int aspectRatioY, int reqWidth, int reqHeight, boolean flipHorizontally,
            boolean flipVertically, int sampleMulti
    ) {
        ...
        int sampleSize = 1;
        try {
            BitmapSampled bitmapSampled = decodeSampledBitmapRegion(
                    context, loadedImageUri, rect, width, height, sampleMulti
            );
        ...
        if (result != null) {
            try {
                result = rotateAndFlipBitmapInt(
                        result, degreesRotated, flipHorizontally, flipVertically
                );
            ...
            return cropBitmap(
                    context, loadedImageUri, points, degreesRotated,
                    fixAspectRatio, aspectRatioX, aspectRatioY,
                    sampleMulti, rect, width, height, flipHorizontally, flipVertically
            );
        }
    }
    ...
    private static BitmapSampled cropBitmap(
            Context context, Uri loadedImageUri, float[] points, int degreesRotated,
            boolean fixAspectRatio, int aspectRatioX, int aspectRatioY, int sampleMulti,
            Rect rect, int width, int height, boolean flipHorizontally, boolean flipVertically
    ) {
        Bitmap result = null;
        int sampleSize;
        try {
            BitmapFactory.Options options = new BitmapFactory.Options();
            options.inSampleSize = sampleSize = sampleMulti * calculateInSampleSizeByRequestedSize(
            ...
            if (fullBitmap != null) {
                try {
                    ...
                    result = cropBitmapObjectWithScale(
                            fullBitmap, points2, degreesRotated,
                            fixAspectRatio, aspectRatioX, aspectRatioY, 1,
                            flipHorizontally, flipVertically
                    );

                } finally {
                    if (result != fullBitmap) fullBitmap.recycle();
                }
            }
        } catch (OutOfMemoryError e) {
            if (result != null) result.recycle();
            ...
            throw new RuntimeException("Failed to load sampled bitmap: "
                    + loadedImageUri + "\r\n" + e.getMessage(), e
            );
        }
        return new BitmapSampled(result, sampleSize);
    ...
    private static BitmapFactory.Options decodeImageForOption(
            @NonNull ContentResolver resolver, Uri uri
    ) throws FileNotFoundException {
    ...
    private static Bitmap decodeImage(
            @NonNull ContentResolver resolver, Uri uri, BitmapFactory.Options options
    ) throws FileNotFoundException {
        do {
            InputStream stream = null;
            try {
    ...
    private static BitmapSampled decodeSampledBitmapRegion(
            Context context, Uri uri, Rect rect,
        ...
        try {
            ...
            if (stream != null) decoder = BitmapRegionDecoder.newInstance(
                    stream, false
            );

            do {
                try {
                    if (decoder != null) return new BitmapSampled(
    ...
    private static Bitmap cropForRotatedImage(
            Bitmap bitmap, float[] points, Rect rect, int degreesRotated,
            boolean fixAspectRatio, int aspectRatioX, int aspectRatioY
    ) {
        if (degreesRotated % 90 != 0) {
            ...
            int compareTo = degreesRotated < 90 ||
                    (degreesRotated > 180 && degreesRotated < 270)
                    ? rect.left
                    : rect.right;
            ...
            if (fixAspectRatio) fixRectForAspectRatio(
                    rect, aspectRatioX, aspectRatioY
            );
            ...
            if (bitmapTmp != bitmap) bitmapTmp.recycle();
        }
        return bitmap;
    }
    ...
    private static int calculateInSampleSizeByMaxTextureSize(int width, int height) {
        int inSampleSize = 1;
        if (mMaxTextureSize == 0) mMaxTextureSize = getMaxTextureSize();
        if (mMaxTextureSize > 0) while ((height / inSampleSize) > mMaxTextureSize
