...
public class CropOverlayView extends View {
    ...
    public void setBounds(float[] boundsPoints, int viewWidth, int viewHeight) {
        if (boundsPoints == null || !Arrays.equals(mBoundsPoints, boundsPoints)) {
            if (boundsPoints == null) Arrays.fill(mBoundsPoints, 0);
            else System.arraycopy(
                    boundsPoints, 0, mBoundsPoints,
                    0, boundsPoints.length
            );
            mViewWidth = viewWidth;
            mViewHeight = viewHeight;
            RectF cropRect = mCropWindowHandler.getRect();
            if (cropRect.width() == 0 || cropRect.height() == 0) initCropWindow();
        }
    }
    ...
    public void setCropWindowLimits(
            ...
            float scaleFactorWidth, float scaleFactorHeight
    ) {
    ...
    private void initCropWindow() {
        ...
        if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) {
            ...
            rect.left = leftLimit + mInitialCropWindowRect.left
                    / mCropWindowHandler.getScaleFactorWidth();

            rect.top = topLimit + mInitialCropWindowRect.top
                    / mCropWindowHandler.getScaleFactorHeight();
    ...
    private void fixCropWindowRectByRules(@NonNull RectF rect) {
