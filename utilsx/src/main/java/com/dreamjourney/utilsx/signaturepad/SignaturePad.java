package com.dreamjourney.utilsx.signaturepad;
...
import com.dreamjourney.utilsx.signaturepad.utils.Bezier;
import com.dreamjourney.utilsx.signaturepad.utils.ControlTimedPoints;
import com.dreamjourney.utilsx.signaturepad.utils.SvgBuilder;
import com.dreamjourney.utilsx.signaturepad.utils.TimedPoint;
import com.dreamjourney.utilsx.signaturepad.utils.ViewConfig;

import java.util.ArrayList;
import java.util.List;

public class SignaturePad extends View {
    private static final String TAG = "Signature_Log";

    public interface OnUndoRedoListener {
        void onUndoRedoStateChanged(boolean canUndo, boolean canRedo);
    }

    private final List<Bitmap> mUndoStack = new ArrayList<>();
    private final List<Bitmap> mRedoStack = new ArrayList<>();
    ...
    public SignaturePad(Context context, AttributeSet attrs) {
        ...
        try (TypedArray a = context.getTheme().obtainStyledAttributes(
            ...
            mMinWidth = a.getDimensionPixelSize(
                    R.styleable.SignaturePad_penMinWidth, convertDpToPx(3)
            );
            mMaxWidth = a.getDimensionPixelSize(
                    R.styleable.SignaturePad_penMaxWidth, convertDpToPx(7)
            );
            mPaint.setColor(a.getColor(R.styleable.SignaturePad_penColor, Color.BLACK));
            mVelocityFilterWeight = a.getFloat(
                    R.styleable.SignaturePad_velocityFilterWeight, 0.9f
            );
            mClearOnDoubleClick = a.getBoolean(
                    R.styleable.SignaturePad_clearOnDoubleClick, false
            );
    ...
    public void setPenColorRes(@NonNull Context context, int colorRes) {
        try {
            setPenColor(context.getColor(colorRes));
    ...
    protected void onDraw(@NonNull Canvas canvas) {
        if (mSignatureBitmap != null) canvas.drawBitmap(mSignatureBitmap, 0, 0, mPaint);
    }
        ...
        // View not laid out yet e.g. called from onCreate(), onRestoreInstanceState()...
            ...
            getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    ...
                    ViewConfig.removeOnGlobalLayoutListener(getViewTreeObserver(), this);
    ...
    private ControlTimedPoints calculateCurveControlPoints(
            @NonNull TimedPoint s1, @NonNull TimedPoint s2, @NonNull TimedPoint s3
    ) {
        float dx1 = s1.x - s2.x;
        float dy1 = s1.y - s2.y;
        float dx2 = s2.x - s3.x;
        ...
        return mControlTimedPointsCached.set(getNewPoint(
                m1X + tx, m1Y + ty), getNewPoint(m2X + tx, m2Y + ty)
        );
    }
    ...
    private void saveBitmapToUndoStack() {
        ensureSignatureBitmap();
        mUndoStack.add(mSignatureBitmap.copy(Bitmap.Config.ARGB_8888, true));
        mRedoStack.clear(); // stroke redo
        // Update Undo Redo
        notifyUndoRedoState();
    }
    ...
    public void undo() {
        ...
        if (!mUndoStack.isEmpty()) {
            ensureSignatureBitmap();
            mRedoStack.add(mSignatureBitmap.copy(Bitmap.Config.ARGB_8888, true));
            mSignatureBitmap = mUndoStack.remove(mUndoStack.size() - 1);
            mSignatureBitmapCanvas = new Canvas(mSignatureBitmap);
            invalidate();
        }
    ...
    public void redo() {
        ...
        if (!mRedoStack.isEmpty()) {
            ensureSignatureBitmap();
            mUndoStack.add(mSignatureBitmap.copy(Bitmap.Config.ARGB_8888, true));
            mSignatureBitmap = mRedoStack.remove(mRedoStack.size() - 1);
            mSignatureBitmapCanvas = new Canvas(mSignatureBitmap);
            invalidate();
        }
    ...
    public void setOnUndoRedoListener(OnUndoRedoListener listener) {
        this.onUndoRedoListener = listener;
    }

    private void notifyUndoRedoState() {
        if (onUndoRedoListener != null) {
            onUndoRedoListener.onUndoRedoStateChanged(
                    !mUndoStack.isEmpty(),
                    !mRedoStack.isEmpty()
            );
        }
